...
movie_point = [[0,0], [0,frame_x], [frame_y,frame_x]] #左上、左下、右下、右上 (Affine transform 只需三個點，but老師要四個點來做) 
background_point = []
...
for index, num in enumerate(background_point):
    for i in range(len(background_point[index])):
        # print(background_point[index][i])
...
for i in range(init.shape[0]):
    if (i%2) == 0:
        init[i][0:2] = movie_point[x][0:2]
        init[i][2] = 1
        count += 1
    else:
        init[i][3:5] = movie_point[x][0:2]
        init[i][5] = 1 
        count += 1  
    if count == 2:
        x += 1
...
calculate_answer = init_inv.dot(target)
...
while(True):
    print('抓取照片')
    ret, frame = cap.read()
    final = aff_wrap_pic(frame, background_pic_1, calculate_answer_inv)   
    print('showing.....')
